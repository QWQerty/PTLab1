...
class TestJsonDataReader(unittest.TestCase):
    ...
    def test_read_successful(self):
        ...
        with patch("builtins.open", mock_open(read_data='{"students": [{"name": "Александра", "rating": 90}]}')):
            result = self.json_reader.read("fake_path.json")
        
        expected_result = {'students_in_last_quartile': [{'name': 'Александра', 'rating': 90}]}
    ...
    def test_read_file_read_error(self):
        ...
        with patch("builtins.open", side_effect=Exception("Unable to read file")):
            result = self.json_reader.read("fake_path.json")
    ...
    def test_filter_students_last_quartile(self):
        ...
        self.assertEqual(set(map(frozenset, result)), set(map(frozenset, expected_result)))
